...
import org.the3deer.engine.services.collada.entities.Controller;
import org.the3deer.engine.services.collada.entities.EffectData;
import org.the3deer.engine.services.collada.entities.Geometry;
import org.the3deer.engine.services.collada.entities.MaterialData;
...
public class ColladaLoader {
    ...
    public Scene load(URI url) throws Exception {
        ...
        try (InputStream stream = url.toURL().openStream()) {
            ...
            Map<Node, org.the3deer.engine.model.Node> nodeMap = new HashMap<>();
            ...
            for (Node parserNode : parser.getNodeLibrary().values()) { // Iterate through ALL parser nodes
                ...
                if (parserNode.getInstanceControllerId() != null && parserNode.getSkinId() != null) {
                    ...
                    if (template instanceof AnimatedModel) {
                        ...
                        org.the3deer.engine.model.Node skeletonRootNode = findNodeInMap(skeletonRootId, nodeMap);
    ...
    , Map<String, Material> materials) {
        ...
        List<org.the3deer.engine.model.Node> rootModelNodes = new ArrayList<>();
     ...
     */
    private org.the3deer.engine.model.Node buildModelNode(Node parserNode,
                                                          org.the3deer.engine.model.Node parentModelNode,
                                                          Map<Node, org.the3deer.engine.model.Node> nodeMap,
                                                          Map<String, Material> materials){
    // If we've already created this node, return the cached version.
        ...
        org.the3deer.engine.model.Node modelNode = new org.the3deer.engine.model.Node(parserNode.getId());
        ...
        for (Node childParserNode : parserNode.getChildren()) {
            org.the3deer.engine.model.Node childModelNode = buildModelNode(childParserNode, modelNode, nodeMap, materials);
    ...
    private void buildScene(Node parserNode, float[] parentMatrix,
        ...
        if (template != null) {
            ...
            org.the3deer.engine.model.Node parentModelNode = nodeMap.get(parserNode);
            newInstance.setParentNode(parentModelNode);

            // Find the engine Node that represents the root of the skeleton (e.g., the "Torso" node)
            final String skinId = parserNode.getSkinId();
            if (skinId != null) {
                org.the3deer.engine.model.Node skeletonRootNode = findNodeInMap(skinId, nodeMap);
                if (skeletonRootNode != null) {
                    // Attach the actual Skin object to the skeleton's root Node.
     ...
     */
    private org.the3deer.engine.model.Node findNodeInMap(String nodeId, Map<Node, org.the3deer.engine.model.Node> nodeMap) {
        if (nodeId == null || nodeMap == null) {
            return null;
        }

        // We iterate through the DTO->Engine Node map to find the one with the matching ID.
        for (org.the3deer.engine.model.Node modelNode : nodeMap.values()) {
