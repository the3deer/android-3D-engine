...
public class Widget extends Object3D implements EventListener {
    ...
    /**
     * Called by the {@link BeanManager}
     */
    @BeanInit
    public void setUp() {
    ...
    public float[] getViewMatrix() {
        ...
        return ((Camera) BeanManager.getInstance().get("gui.camera")).getViewMatrix();
    }

    public float[] getProjectionMatrix() {
        //return Configuration.getInstance().projectionMatrix;
        return ((Camera) BeanManager.getInstance().get("gui.camera")).getProjectionMatrix();
    }
...
/*    public float[] getViewportSize() {
     ...
     * @return this object
     */
    public Object3D setBindScale(float[] scale) {
    ...
    protected void refreshRelativeLocation() {
        ...
        // Safe area calculation (normalize insets/UI heights to OpenGL coordinates -1 to 1)
        float topOffset = 0;
        float bottomOffset = 0;
        if (parent instanceof GUI) {
            if (screen != null) {
                // We respect both the system safe area and the explicit Toolbar/BottomBar heights
                int totalTop = screen.getTop() + screen.getToolbarHeight();
                int totalBottom = screen.getBottom() + screen.getBottomBarHeight();
                
                topOffset = (float) totalTop / screen.getHeight() * 2.0f;
                bottomOffset = (float) totalBottom / screen.getHeight() * 2.0f;
            }
        }
        ...
        switch (relativeLocation) {
            case POSITION_TOP_LEFT:
                ...
                y = parentDim.getMax()[1] - widgetDim.getMax()[1] - topOffset;
                break;
            case POSITION_TOP:
            ...
            case POSITION_BOTTOM:
                x = parentDim.getCenter()[0] - widgetDim.getCenter()[0];
                y = parentDim.getMin()[1] - widgetDim.getMin()[1] + bottomOffset;
                break;
...
//        return Configuration.getInstance().screenWidth;
        return ((Screen) BeanManager.getInstance().get("screen")).getWidth();
    }

    public int getScreenHeight() {
        //return Configuration.getInstance().screenHeight;
        return ((Screen) BeanManager.getInstance().get("screen")).getHeight();
    }
    ...
    public void removeChild(Widget child) {
        ...
        propagate(new ChildRemoved(child));
    }
